...
module.exports = function(options){
	...
	console.log("options in utils ", options);
	...
	return {
		...
		create: function(callback){
			var self = this,
				sid = uuid.v4();
			var token = jwt.sign({ iss: sid }, options.secret, { algorithm: options.algorithm });
			console.log("creating session with key", options.keyspace, sid);
