...
module.exports = function(options){
	...
	var expSeconds = Math.floor(options.expires / 1000);
	...
	return {
		...
		create: function(callback){
			...
			options.client.setex(options.keyspace + sid, expSeconds, JSON.stringify(serializeSession(self)), function(error){
				self.id = sid;
		...
		touch: function(callback){
			...
			options.client.expire(options.keyspace + this.id, expSeconds, callback);
		},
		...
		update: function(callback){
			if(!this.id)
				return callback(new Error("Invalid session ID"));
			options.client.setex(options.keyspace + this.id, expSeconds, JSON.stringify(serializeSession(this)), callback);
		},
